...
(0, _mocha.describe)("User Login", function () {
    (0, _mocha.it)("It should return the token of username and password", function (done) {
        _chai2.default.request(_app2.default).post(login).send({ username: "lalit", password: "lalit" }).end(function (err, res) {
    ...
    (0, _mocha.it)("It should return error if username property does not exist", function (done) {
        _chai2.default.request(_app2.default).post(login).send({ password: "lalit" }).end(function (err, res) {
    ...
    (0, _mocha.it)("it should return error if password property does not exists", function (done) {
        _chai2.default.request(_app2.default).post(login).send({ username: "lalit" }).end(function (err, res) {
            res.should.have.status(400);
            res.body.should.have.property("message");
            done();
        });
...
(0, _mocha.describe)("authorize", function () {
    (0, _mocha.it)("should return the username and password if correct token is given", function (done) {
        var token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImxhbGl0IiwicGFzc3dvcmQiOiJsYWxpdCIsImlhdCI6MTUxODk3MjE0OH0.DVunGKusnzUm6teNNz5UGMZ1OXh6USFRU09SAaxO6zA";
